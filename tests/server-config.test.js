...
test("getPresetServerConfig returns expected built-in endpoints", () => {
  ...
  assert.equal(osm.defaultBaseLayer.tileUrl, "https://tile.openstreetmap.org/{z}/{x}/{y}.png");
  assert.equal(ohm.siteUrl, "https://www.openhistoricalmap.org");
  assert.equal(ogf.apiBase, "https://opengeofiction.net/api/0.6/");
  assert.equal(ogf.defaultBaseLayer.name, "OpenGeofiction");
  assert.equal(ogf.defaultBaseLayer.tileUrl, "https://tiles05.opengeofiction.net/ogf-carto/{z}/{x}/{y}.png");
});
...
test("normalizeServerConfig keeps custom overrides normalized", () => {
  ...
  assert.deepEqual(config.defaultBaseLayer, {
    name: "Example tiles",
    tileUrl: "https://tiles.example.test/{z}/{x}/{y}.png",
    attribution: "Example attribution"
  });
